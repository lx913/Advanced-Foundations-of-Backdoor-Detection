...
if __name__ == '__main__':
    ...
    for i in range(args.modelnum):
        if args.model == 'resnet':
            ...
            model = SENet18()
        model.load_state_dict(torch.load('save_model/'+args.model+'_fine-tuning_0.05/model_'+str(i+1)+'.pth'))
        model = model.cuda()
        ...
        if args.model == 'resnet':
            ...
            model = SENet18()
        model.load_state_dict(torch.load('save_model/'+args.model+'/model_'+str(i+1)+'.pth'))
        model = model.cuda()
