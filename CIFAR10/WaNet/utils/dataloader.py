...
def get_dataloader(opt, train=True, pretensor_transform=False):
    ...
    if opt.dataset == "gtsrb":
        if train == True:
            ...
            split="test"
        dataset = torchvision.datasets.GTSRB(root=opt.data_root,split=split,transform=transform,download=True)
    elif opt.dataset == "mnist":
...
def get_dataloader_fine(opt, train=True, pretensor_transform=False):
    transform = get_transform(opt, train, pretensor_transform)
    if opt.dataset == "gtsrb":
        ...
        dataset = torchvision.datasets.CIFAR10(opt.data_root, train, transform, download=True)
        len_train = len(dataset) // 10 
        len_train2 = len(dataset) - len_train
        dataset, dataset2 = random_split(dataset, [len_train,len_train2])
