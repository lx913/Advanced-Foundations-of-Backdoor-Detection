...
def load_init_data(dataname, download, dataset_path):
    ...
    train_data = torchvision.datasets.CIFAR10(root=dataset_path, train=True, download=download)
    test_data = torchvision.datasets.CIFAR10(root=dataset_path, train=False, download=download)
...
def create_backdoor_data_loader(dataname, train_data, test_data, trigger_label, posioned_portion, atk, batch_size):
    if dataname == 'cifar10':
        ...
        test_data_clean = PoisonedCIFAR10(test_data, trigger_label, portion=0, mode="test", patch_size = args.patch_size, atk=args.atk, dataname=dataname)
        test_data_poison = PoisonedCIFAR10(test_data, trigger_label, portion=1, mode="test", patch_size = args.patch_size, atk=args.atk, dataname=dataname)
        ...
        test_data_poison_loader = DataLoader(dataset=test_data_poison, batch_size=batch_size, num_workers=8, shuffle=True)
...
if __name__ == "__main__":
    ...
    print("# --------------------------"+args.atk+" models fine-tuning")
    best_performance = []
    for num in range(args.modelnum):
        ...
        if args.model == 'resnet':
            ...
            model =SENet18()
        model.load_state_dict(torch.load('save_model/'+args.model+'/model_portion0.1_'+str(num+1)+'.pth'))
