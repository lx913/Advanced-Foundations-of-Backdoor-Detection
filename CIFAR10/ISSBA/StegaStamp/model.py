...
class StegaStampEncoder(nn.Module):
    def __init__(self):
        ...
        self.secret_dense = Dense(100, 192, activation='relu', kernel_initializer='he_normal')
    ...
    def forward(self, inputs):
        ...
        secrect = secrect.reshape(-1, 3, 8, 8)
...
def build_model(encoder, decoder, discriminator, lpips_fn, secret_input, image_input, l2_edge_gain,
    ...
    input_warped = torchgeometry.warp_perspective(image_input, M[:, 1, :, :], dsize=(32, 32), flags='bilinear')
    mask_warped = torchgeometry.warp_perspective(torch.ones_like(input_warped), M[:, 1, :, :], dsize=(32, 32),
                                                 flags='bilinear')
    ...
    residual = torchgeometry.warp_perspective(residual_warped, M[:, 0, :, :], dsize=(32, 32), flags='bilinear')

    if borders == 'no_edge':
        encoded_image = image_input + residual
    elif borders == 'black':
        encoded_image = residual_warped + input_warped
        encoded_image = torchgeometry.warp_perspective(encoded_image, M[:, 0, :, :], dsize=(32, 32), flags='bilinear')
        input_unwarped = torchgeometry.warp_perspective(image_input, M[:, 0, :, :], dsize=(32, 32), flags='bilinear')
    elif borders.startswith('random'):
        mask = torchgeometry.warp_perspective(torch.ones_like(residual), M[:, 0, :, :], dsize=(32, 32),
                                              flags='bilinear')
        encoded_image = residual_warped + input_unwarped
        encoded_image = torchgeometry.warp_perspective(encoded_image, M[:, 0, :, :], dsize=(32, 32), flags='bilinear')
        input_unwarped = torchgeometry.warp_perspective(input_warped, M[:, 0, :, :], dsize=(32, 32), flags='bilinear')
    ...
    for i in range(int(falloff_im.shape[0] / falloff_speed)):  # for i in range 100
        falloff_im[-i, :] *= (np.cos(4 * np.pi * i / size[0] + np.pi) + 1) / 2  # [cos[(4*pi*i/32)+pi] + 1]/2
        falloff_im[i, :] *= (np.cos(4 * np.pi * i / size[0] + np.pi) + 1) / 2  # [cos[(4*pi*i/32)+pi] + 1]/2
